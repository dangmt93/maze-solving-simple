...
void generate_levelBF(game_tree t, queue q)
{
	...
	{
		// Calculate new coordiates from current position (orginal game_state)
		x = get_current_row(s) + VERT_MOVES[count];
		y = get_current_column(s) + HORIZ_MOVES[count];

		n = clone(s); // clone original game_state (so changes will not affect original state)

		// If able to move -> attach to new branch to initial game_tree + add branch to queue
		if(move(n, x, y))
		{
			init_game_tree(&c, false, n);
			
			// determine where to attach new t_node
			...
			{
				if(is_empty_game_tree(get_middle(t)))
				{
					set_middle(t, c);
				}
				else
			...
			// set parent for newly attached branch
			set_parent(c, t);
			
			// Add branch (game_tree pointing to new move) to queue
			add(q, c);
		}
	}
...
game_tree build_gameBF(game_tree t, queue q)
{
	...
	// Add initial game_tree to queue
	c = t;
	add(q, c);

	while(!is_empty_queue(q) && !solved(get_data(c)))
	{
		c = (game_tree)front(q); 	// retrieve next game_tree in queue
		rear(q); 					// remove front of queue
		generate_levelBF(c, q); 	// generate another level from given branch c
	}

	// If queue not empty (found solution) -> return final game_state
	// else make c empty tree + return
	if(!is_empty_queue(q))
	{
		return c; // return final game_state
	}
	else
	{
		c->root = NULL;
...
void generate_levelDF(game_tree t, stack k)
{
	...
	for(count = 0; count < MOVE_COUNT; count++)
	{
		...
		// If able to move -> attach to new branch to initial game_tree + push branch to stack
		if(move(n, x, y))
		{
			init_game_tree(&c, false, n);
			...
			// Push branch (game_tree pointing to new move) to stack
			push(k, c);
		}
	}
...
game_tree build_gameDF(game_tree t, stack k)
{
	...
	// Push initial game_tree to stack
	c = t;
	push(k, c);

	while(!is_empty_stack(k) && !solved(get_data(c)))
	{
		c = (game_tree)top(k);		// retrieve next game_tree in stack
		pop(k);						// pop top of stack
		generate_levelDF(c, k); 	// generate another level from given branch c
	}


	// If stack not empty (found solution) -> return final game_state
	// else make c empty tree + return
	if(!is_empty_stack(k))
	{
		return c; // return final game_state
